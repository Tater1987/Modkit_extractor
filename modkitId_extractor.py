...
class CarcolsMetaExtractor:
    def __init__(self, base_directories=None):
        ...
        if base_directories is None:
            self.base_directories = ["emergency"] #Edit your folder name here if you have one folder!!!!!!!!!!!
    ...
    def extract_id_values(self, file_path):
        """Extract all kitName values from within <Kits> to </Kits> sections"""
        id_values = []
        ...
        try:
            ...
            # Find all <Kits> to </Kits> sections
            kits_pattern = r'<Kits>(.*?)</Kits>'
            kits_sections = re.findall(kits_pattern, content, re.DOTALL)
            
            for section in kits_sections:
                # Find all <kitName> patterns that end with _modkit
                # Example: <kitName>7853_coquette4_modkit</kitName>
                kitname_pattern = r'<kitName>([^<]*_modkit)</kitName>'
                kitname_matches = re.findall(kitname_pattern, section, re.IGNORECASE)
                id_values.extend(kitname_matches)
